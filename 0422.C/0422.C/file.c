...
int main() {
	// 파일 불러오기 :: fopen()
	// r :: 읽기 모드
	// w :: 쓰기 모드 (txt 파일 내용 새로 집어넣기) 처음부터 새로 작성(기존의 내용이 삭제)
	// w+ :: 읽기+쓰기 모드 (읽기랑 쓰기 다 가능), 파일이 있으면 새로운 데이터로 덮어씌워짐(w와 유사)
	// a :: 추가 모드, 파일이 있으면 끝에 데이터 추가 (기존 파일 내용에 새로 추가)
	...
	if (fp != NULL) {		// fp 변수에 어떠한 정보라도 받았다면
		...
		// fputc(문자정보, FILE*) 함수 :: 문자 하나씩 파일에 입력
		fputc('a', fp);
		fputc('b', fp);
		fputc('c', fp);
